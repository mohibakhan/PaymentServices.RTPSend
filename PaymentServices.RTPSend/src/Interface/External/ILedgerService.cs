...
/// <summary>
/// No-op implementation that always succeeds with a generated reservation ID.
/// Replace with the real implementation when the NuGet package is available.
/// </summary>
public sealed class NoOpLedgerService : ILedgerService
{
    public Task<LedgerReservationResult> ReserveAsync(LedgerReservationRequest request, CancellationToken cancellationToken = default) =>
        Task.FromResult(LedgerReservationResult.Ok(Guid.NewGuid().ToString("N")));
}
