...
public class RtpSendSettings
{
    ...
    // -------------------------------------------------------------------------
    // Note: RETRY_TIMER_SCHEDULE is intentionally NOT here. The timer trigger
    // reads it via "%RETRY_TIMER_SCHEDULE%" — the Functions host resolves
    // that from a flat env var before the worker starts. Worker code never
    // needs to read this value.
    // -------------------------------------------------------------------------
}
