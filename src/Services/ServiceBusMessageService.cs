...
public sealed class ServiceBusMessageService : IServiceBusMessageService
{
        ...
        TimeSpan? delay = null) where T : notnull
    {
        ...
        var scheduledFor = delay is { } d && d > TimeSpan.Zero
            ? DateTimeOffset.UtcNow.Add(d)
            : (DateTimeOffset?)null;

        _logger.LogInformation(
            "Sending message to {Queue} with subject {Subject} (scheduledFor {ScheduledFor}): {Content}",
            queueOrTopicName, subject ?? "(none)", scheduledFor?.ToString("o") ?? "now", content);
