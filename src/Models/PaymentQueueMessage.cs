...
/// <summary>
/// Envelope on the <c>rtpsend-tabapay-retry</c> queue. Enqueued (with a backoff
/// delay) when a TabaPay send fails transiently, so the call is retried later
/// instead of redelivering the outcome message instantly. <see cref="Attempt"/>
/// is the 1-based retry number; it lives in the body because a freshly-scheduled
/// message resets the Service Bus DeliveryCount.
/// </summary>
public sealed class TabaPayRetryMessage
{
    [JsonPropertyName("evolveId")]
    public string EvolveId { get; init; } = string.Empty;
