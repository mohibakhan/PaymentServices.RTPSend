...
///   3. If the doc shows a terminal status (COMPLETED or FAILED_NSF),
///      complete the DLQ message without re-processing
///   4. Otherwise hand to <see cref="IPaymentOrchestrator.ResumeFromAsync"/>,
///      which inspects payment.Stage and resumes from the failed step only
///   5. Complete the DLQ message on success; abandon on transient failure
/// </summary>
public class RetryFailedPayments
{
    ...
    public async Task Run(
        [TimerTrigger("%RETRY_TIMER_SCHEDULE%")] TimerInfo timerInfo,
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Terminal states — never retry. This should be rare (ProcessPayment's
        // NSF catch completes the SB message before it reaches DLQ), but defends
        // against legacy entries or race conditions where the Cosmos patch
        // succeeded but the SB ack didn't.
        if (payment.Status == nameof(RequestStatus.FAILED_NSF))
        {
            _logger.LogInformation(
                "Skipping DLQ retry for evolveId {EvolveId} — terminal NSF status.",
                envelope.EvolveId);
            await receiver.CompleteMessageAsync(sbMessage, cancellationToken);
            return;
        }

        if (payment.Status == nameof(RequestStatus.FAILED_TABAPAY))
        {
            // Non-retryable TabaPay decline/validation — resuming would just
            // re-fail every timer cycle. Drop it from the DLQ for investigation.
            _logger.LogInformation(
                "Skipping DLQ retry for evolveId {EvolveId} — terminal TabaPay failure.",
                envelope.EvolveId);
            await receiver.CompleteMessageAsync(sbMessage, cancellationToken);
            return;
        }

        if (payment.Status == nameof(RequestStatus.COMPLETED))
        {
            _logger.LogInformation(
                "Skipping DLQ retry for evolveId {EvolveId} — already completed.",
                envelope.EvolveId);
            await receiver.CompleteMessageAsync(sbMessage, cancellationToken);
            return;
        }
